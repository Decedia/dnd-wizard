...
| 2026-08-24 | Reordered creation wizard steps to: Identity → Class → Race → Abilities; updated StepIdentity to use dropdown for Background (SRD backgrounds from srd.ts) and removed Player Name; rebuilt StepAbilities with tabbed methods (Standard Array, Point Buy, Dice Roll); Dice Roll tab is manual entry only (no website dice rolling) with +/- buttons, min 8, max 15; Point Buy uses 27 points with proper state-driven UI updates; Standard Array uses fixed [15,14,13,12,10,8] with dropdown assignment and race modifier shown left of dropdown; lint and typecheck pass |
