...
### Localization
- **LanguageContext**: `src/contexts/LanguageContext.tsx` provides 300+ translation keys for `en` and `id`; `useLanguage().t(key, fallback)` pattern used throughout
...
- **Coverage**: All user-facing strings in character sheet (14 sections), creation wizard (10 steps), modals, app pages, and navigation use `t()` for localization
...
- Updated spell/feature/trait summaries: increased from 12 to 30 words max, added mechanism details (action type, uses, duration, targeting, resolution, effects, concentration); added spell summaries to Character type
