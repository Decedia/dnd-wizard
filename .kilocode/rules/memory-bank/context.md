...
- [x] Redesigned equipment selection modal UX: concrete options now render as single icon buttons (primary item icon + description + quantity), weapon/focus choices show scrollable item lists directly under the "or" divider instead of expandable buttons, mixed options (category + concrete item like "martial weapon and a shield") show disabled visual buttons for bonus items at the top of the list, and multi-select weapon lists show a live "X/N selected" counter; updated `EquipmentChoiceModal.tsx` and passed `getItemInfo` from `StepEquipment.tsx`; typecheck passes, lint only pre-existing errors
- Pending: character deletion from home screen (already implemented), PDF export/import, database persistence via add-database recipe
- Future: Refactor shared step rendering between PerLevelStepsFlow and LevelUpFlow to reduce duplication, add more PHB subclass features to subclass JSON entries, add more class feature choice options
