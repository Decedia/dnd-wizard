...
- [x] Unified popup refactor: created `BasePopup` component with standardized header (title + optional info button + close), content area, and footer (Cancel + Confirm buttons); refactored InfoButton, DescriptionModal, FeatSelector, SpellSelectionModal, StepOrigin class/race popups to use BasePopup; removed per-item info buttons from selection lists per new UX rules; typecheck passes
