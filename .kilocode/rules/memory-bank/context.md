...
- [x] Built `LanguageToggle.tsx` component for switching between English and Indonesian; integrated into `AppHeader`; wrapped app root in `LanguageProvider` via `layout.tsx`
- [x] Translated all user-facing strings across the app: `BottomNav`, `WizardNav`, home page, admin page, dice page, character sheet page, and all 14 character sheet section components (`StatsSection`, `CombatStatsSection`, `PassiveStatsSection`, `DeathSavesSection`, `HitDiceSection`, `SkillsSection`, `FeaturesTraitsSection`, `OtherProficienciesSection`, `SpellsSection`, `SpellcastingStatsSection`, `AttacksAndSpellcastingSection`, `InventorySection`, `CurrencySection`, `LevelXpSection`, `AppearanceBioSection`, `IdentitySection`) now use `t()` for section titles, buttons, status labels, placeholders, and form labels
- [x] Translated character creation wizard steps: `StepIdentity`, `StepRace`, `StepClass`, `StepFeatureSelections`, `StepPersonality`, `StepLevel`, `StepOrigin`, `StepAppearance`, `StepSkills`, `StepSourceSelection` all use `useLanguage().t()` for titles, hints, button labels, and form fields
- [x] Translated modals and popups: `CustomItemModal`, `SubclassDetailsModal`, `InfoButton`/`DescriptionModal`, `CombatStatsSection` HP modal all use `t()` for titles, confirm/cancel labels, and form placeholders
- [x] Translated app pages: `src/app/character/create/page.tsx` (wizard next label), `src/app/character/[id]/level-up/page.tsx` (loading/error states), `src/app/admin/srd-data/page.tsx` (admin access, back button, test runner labels)
- [x] Added 50+ new translation keys to `LanguageContext.tsx` covering namespaces: `section.*`, `button.*`, `status.*`, `form.*`, `placeholder.*`, `creator.*`, `wizard.*`, `modal.*`, `admin.*`, `character.*`, `spell.*`, `equipment.*`, `common.*` with full Indonesian translations
- [x] Fixed duplicate translation keys in `LanguageContext.tsx` that were causing typecheck errors; verified `bun typecheck` and `bun lint` pass cleanly

- [x] Updated spell/feature/trait summaries from 12 words max to 30 words max with mechanism inclusion: changed feature summary generation in `src/lib/storage.ts` to use 30 words and append mechanism info (action type, uses, requirement, duration, onUse, scaling); added `summary` field to spells in Character interface; implemented spell summary generation from spell mechanics data (effect types, resolution, targeting, duration, concentration); typecheck and build pass
...
- [x] Unified popup refactor: created `BasePopup` component with standardized header (title + toggleable info button + close), content area, and footer (Cancel + Confirm buttons); refactored InfoButton, DescriptionModal, FeatSelector, SpellSelectionModal, StepOrigin class/race popups, FeatPopup, ItemSelectionPopup, StepPersonality, StepLevel ASI modal, CombatStatsSection HP modal to use BasePopup; removed per-item info buttons from selection lists per new UX rules; info button in header toggles an info description section; typecheck passes
- [x] Modal consolidation + extraction: refactored all remaining LevelUpWizard inline modals (ASI, Spell Mastery, Signature Spells, Feature Selection, Humanoid Races, Terrain, Bonus Cantrip) and existing modal components (SubclassDetailsModal, SubclassSelectionModal, SpellSelectionModal, FeatPopup→FeatModal, ItemSelectionPopup→ItemSelectionModal, FeatSelector→FeatSelectionModal) into `src/components/modals/` with `Modal` suffix naming; all now use BasePopup; fixed broken JSX nesting where `showHumanoidPopup` was nested inside `showFeaturePopup` block; InfoTooltip and SkillsSection popover left unchanged (hover tooltip and small contextual popover, not full-screen modals); typecheck passes, lint only pre-existing errors (ThemeContext setState-in-effect, react-hooks warnings)
- [x] Removed per-item InfoButtons from selection lists: StepEquipment instrument/focus choices, SubclassSelectionModal, FeatureSelectionModal, SpellSelectionModal, BonusCantripModal, TerrainModal, StepSpells, StepRace, StepClass, StepSubclass, StepFeatureSelections option items; StepEquipment selection modal now uses BasePopup directly instead of DescriptionModal; InfoButton retained only in non-selection contexts (armor/item details, feature headers); typecheck passes
- [x] Fixed StepEquipment weapon selection popup to always show BasePopup footer (Cancel + Confirm) for weapon choices, not just multi-select; single weapon selections now also have confirmation buttons
- [x] Repositioned NewPlayerTips to sit above the relevant content sections: StepOrigin tips above class/race selection, StepAbilities tips above ability score generation, StepSkills tips above skill grid; removed duplicate StepCard closing tag in StepSkills; typecheck and lint pass with only pre-existing errors
- [x] Updated D&D icons: added per-tip icons to NewPlayerTips (Sword/Person/Star for Origin, ChartBar/Sparkles/Dice for Abilities, ListChecks/Crown/Book for Skills); expanded Gi icon library with Polearm, Whip, Trident, Musket; added weapon name-to-icon mapping in StepEquipment for specific weapons (Battleaxe, Glaive, Whip, Trident, etc.); added Artificer class icon (GearGiIcon) and Crown subclass icon to FeaturesTraitsSection/StepSubclass; weapon displays now use specific weapon icons
...
- [x] Added `showDescriptions` toggle to character sheet: descriptions hidden by default in FeaturesTraitsSection and SpellsSection; toggle button (Eye/EyeSlash icon) in AppHeader next to ViewEditToggle; preference persisted in localStorage under key `dnd-show-descriptions`; CharacterSheetContext extended with `showDescriptions` and `setShowDescriptions`
- [x] Redesigned skill proficiency indicators in SkillsSection view mode: replaced Star+Circle icons with colored full circles (green for background, purple for expertise, black for normal); added InfoIcon button that toggles a source badge showing "expertise", "background", or "normal +{profBonus}"
- [x] Fixed spell preparation bug in LevelUpWizard: `handleFinish` was using `isPreparationCaster()` (which includes Wizard and Paladin) to auto-mark all selected spells as prepared. Replaced with `["Cleric", "Druid", "Artificer"].includes(draft.class)` to match `buildLevelInfos` `isPrepCaster` logic, so only true preparation casters get auto-prepared spells. Moved `preparedSpells` assignment to after all subclass always-prepared spells (domain, circle) are collected. Removed unused `isPreparationCaster` import.
...
| `src/components/character-sheet/FeaturesTraitsSection.tsx` | Locked feature rendering, action type cycling, usable feature filtering (Use button only for action/bonus/reaction/free features), FeatureMechanicsChips integration with source-colored left borders | ✅ Ready |
| `src/components/character-sheet/FeatureMechanicsChips.tsx` | Strict 2-column always-paired grid for feature cards: Type|Action, Source|Uses, OnUse|Requires, Duration|EndsIf, Scales|blank; BlankCell for empty sides; Active/Passive badges; colored EndsIf/Scales text | ✅ Ready |
...
- [x] Redesigned equipment selection modal UX: concrete options now render as single icon buttons with primary item icon + description, weapon/focus choices show scrollable item lists directly under the "or" divider (no expand button needed), mixed options show disabled visual buttons for bonus items at the top, and multi-select lists show a live "X/N selected" counter; concrete options with multiple items now render a row of item icons side-by-side so all included equipment is visible at a glance; added InfoButton to weapon option buttons and bonus items in the scrollable list; concrete option buttons now show rich item descriptions from `getItemInfo` in their InfoButton instead of just item names; added `generateFallbackDescription` helper in `StepEquipment.tsx` that creates rich descriptions for weapons (damage dice/type/properties), armor (AC/dex bonus/type), and generic items when raw descriptions are missing or too short; updated `EquipmentChoiceModal.tsx` and passed `getItemInfo` from `StepEquipment.tsx` to modal for icon resolution; typecheck passes, lint only pre-existing errors
- [x] Added sourcebook-aware class/subclass filtering in LevelUpWizard: updated `getStaticClass` in `srd-client.ts` to accept `sources` parameter with backward-compatible overload (existing `(name, ruleset)` calls still work); updated `LevelUpWizard.tsx` to pass `character.sources` when looking up class data; updated `SubclassSelectionModal.tsx` to accept `characterSources` prop and pass it to `getStaticSubclasses` so modal details respect the character's selected sourcebooks; character's `sources` field from StepSourceSelection is already persisted and used by subclass/spell filtering; typecheck passes, lint only pre-existing errors
...
- [x] Added test character generator tool: created `src/lib/test-character-generator.ts` with `generateTestCharacters()` that creates one level-5 character per class/subclass combination with standard array abilities + racial bonuses, first skill picks, first equipment options, first cantrips/spells, flat-average HP, and pre-populated feature selections; added `removeTestCharacters()` to delete all "Test"-suffixed characters; updated home screen (`src/app/page.tsx`) with muted secondary "Generate Test Characters" and "Remove Test Characters" buttons at the bottom, confirmation modals, progress bar with current character name, and results summary showing success/failure counts; typecheck passes, lint only pre-existing errors
...
| 2026-09-16 | Added sourcebook-aware class/subclass filtering in LevelUpWizard: updated `getStaticClass` in `srd-client.ts` to accept `sources` parameter with backward-compatible overload (existing `(name, ruleset)` calls still work); updated `LevelUpWizard.tsx` to pass `character.sources` when looking up class data; updated `SubclassSelectionModal.tsx` to accept `characterSources` prop and pass it to `getStaticSubclasses` so modal details respect the character's selected sourcebooks; character's `sources` field from StepSourceSelection is already persisted and used by subclass/spell filtering; typecheck passes |
| 2026-09-16 | Added rich fallback descriptions to equipment selection: concrete option InfoButtons now show the actual item descriptions from `getItemInfo` instead of just item names; weapon list items and bonus items also show InfoButton with descriptions; added `generateFallbackDescription` helper in `StepEquipment.tsx` that creates rich descriptions for weapons (damage dice/type/properties), armor (AC/dex bonus/type), and generic items when raw descriptions are missing or too short; updated `EquipmentChoiceModal.tsx` to use item descriptions for concrete option InfoButtons; typecheck passes, lint only pre-existing errors |
| 2026-09-16 | Added rich fallback descriptions to equipment selection: weapon list items and bonus items now show InfoButton with descriptions; added `generateFallbackDescription` helper in `StepEquipment.tsx` that creates rich descriptions for weapons (damage dice/type/properties), armor (AC/dex bonus/type), and generic items when raw descriptions are missing or too short; updated `EquipmentChoiceModal.tsx` to render InfoButton on weapon option buttons and bonus item buttons using `getItemInfo`; typecheck passes, lint only pre-existing errors |
...
- [x] Applied strict 2-column always-paired grid layout to Feature cards in Features tab matching spell card grid system: created `FeatureMechanicsChips.tsx` component (Type|Action, Source|Uses, OnUse|Requires, Duration|EndsIf, Scales|blank rows with BlankCell for empty sides, Active badge #ebf8ff/#90cdf4/#2b6cb0, Passive badge #f0f0f0/#e0e0e0/#666, EndsIf blue #2b6cb0, Scales purple #6b46c1); integrated into `FeaturesTraitsSection.tsx` below description; added colored left border per source type (race purple #6b46c1, class blue #2b6cb0, subclass green #276749); expanded feature data structures with `featureType`, `actionType`, `uses`, `requirement`, `duration`, `endsIf`, `effect`, `onUse`, `scaling` fields in `2014_classes.json`, `2014_races.json`, `2014_subclasses.json`, `2014_feats.json`; populated sample data for Rage, Second Wind, Sneak Attack, Unarmored Defense; updated `Character` type in `storage.ts`; typecheck passes, lint passes
...
Admin test lab page complete with AI-friendly validation summary. Next steps:
...
| 2026-09-17 | Added test character generator tool: created `src/lib/test-character-generator.ts` with `generateTestCharacters()` that creates one level-5 character per class/subclass combination with standard array abilities + racial bonuses, first skill picks, first equipment options, first cantrips/spells, flat-average HP, and pre-populated feature selections; added `removeTestCharacters()` to delete all "Test"-suffixed characters; gated home screen buttons behind existing `useDebug()` admin mechanism (`enabled && unlocked`) so they only render for admin sessions, placed under a subtle "Admin Tools" label; generation/deletion functions remain module-scoped and not exposed on `window`; typecheck passes, lint only pre-existing errors |
| 2026-09-17 | Added admin test character validation suite: created `src/lib/test-character-validator.ts` with `validateTestCharacter()` covering 11 categories (Ability Scores/Saves, Hit Points, Skills/Proficiency, Features/Traits, Subclass, Spells, Prepared Spells, Ranger Terrain, Fighting Style, Equipment, Completeness); integrated into test generator so validation runs automatically after generation; updated home screen with collapsible validation report UI showing PASS/FAIL/WARNING per check with filter buttons (All/Failures/Warnings/Passing), expected vs actual values, character-level summary, and copy-report button; typecheck passes, lint only pre-existing errors |
| 2026-09-17 | Moved admin test tools to dedicated `/admin` page: created `src/app/admin/page.tsx` with full test character generation and validation UI, gated by existing `useDebug()` admin check (non-admin users see nothing); home screen now shows a single muted "Admin Test Lab" link instead of inline generation/validation UI; added `generateAISummary()` to `test-character-validator.ts` producing a structured plain-text/Markdown report optimized for pasting into AI chat, grouped by failures/warnings/passing characters with expected vs actual values; admin page includes "Copy AI Summary" button; typecheck passes, lint only pre-existing errors |
| 2026-09-18 | Implemented full Bahasa Indonesia (Indonesian) localization: created `LanguageContext.tsx` with 300+ translation keys (`en`/`id`), `LanguageToggle.tsx` component, wrapped app in `LanguageProvider`; translated all 14 character sheet sections, all 10 character creation wizard steps, all modals/popups, and app pages (`/create`, `/character/[id]/level-up`, `/admin/srd-data`) to use `t()` for user-facing strings; added 50+ new translation keys with Indonesian translations; fixed duplicate keys causing typecheck errors; typecheck and lint pass |
| 2026-09-18 | Added Bahasa Indonesia description/summary translations with protected English game terms: updated `tDesc()` in `LanguageContext.tsx` to preserve protected D&D terms (spell names, class names, damage types, etc.) when returning Bahasa translations by scanning English fallback with `protectedTermsRegex`; added example description translations for Acid Splash, Fireball, Darkvision, Fey Ancestry; wired `tDesc` into `SpellsSection.tsx` (spell descriptions), `FeaturesTraitsSection.tsx` (feature descriptions), `FeatModal.tsx` (feat descriptions), and `SubclassDetailsModal.tsx` (subclass/feature descriptions); translation keys follow pattern `spell.desc.{name}`, `feature.desc.{name}`, `feat.desc.{name}`, `subclass.desc.{class}-{subclass}`; typecheck passes |
