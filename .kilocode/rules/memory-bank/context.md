...
| 2026-09-03 | Filtered usable features in FeaturesTraitsSection: removed Use button and USED badge from passive/undefined actionType features; fixed action button icon sizes (h-3 w-3 → h-4 w-4) in FeaturesTraitsSection and SpellsSection; made combat action buttons match heal/damage pill style with grid layout; added showDescriptions toggle (Eye/EyeSlash button in AppHeader, localStorage persistence, hidden by default in FeaturesTraitsSection and SpellsSection) |
