...
| 2026-09-16 | Redesigned equipment selection modal UX: concrete options now render as icon buttons with a row of item icons for all included equipment (not just the first item), weapon/focus choices show scrollable item lists directly visible under the "or" divider (no expand button needed), mixed options show disabled visual buttons for bonus items at the top, and multi-select lists show a live "X/N selected" counter; removed `activeWeaponPicker`/`selectedWeaponInPicker` state from `EquipmentChoiceModal`; passed `getItemInfo` from `StepEquipment` to modal for icon resolution; typecheck passes |
