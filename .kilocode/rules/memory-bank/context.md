...
- [x] Redesigned StepSourceSelection (Step 1) with rich book card grid: created BookCard.tsx component with dark header backgrounds, inline SVG geometric patterns and illustrations per book, checkmark/lock badges, 2-column responsive grid, quick-action pills (PHB only/Select all), selection counter, and Next button; PHB always locked/selected; state persists via wizard flow; typecheck passes
- [x] Created standalone Three.js 3D character test page at `/test-3d` with `TestCharacters3D.tsx` component: 6 class figures (Fighter, Wizard, Rogue, Barbarian, Cleric, Druid) built from BoxGeometry primitives with class-specific materials/accessories; each card has its own WebGL renderer with lighting setup; preview canvas has OrbitControls with auto-rotation and orbiting point light; selected class updates preview with 1.4x scale; temporary "Test 3D" button added to home page; installed three.js and @types/three; typecheck and build pass
