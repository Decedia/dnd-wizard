...
| 2026-08-24 | Reordered creation wizard steps to: Identity → Class → Race → Abilities; updated StepIdentity to use dropdown for Background (SRD backgrounds from srd.ts) and removed Player Name; rebuilt StepAbilities with tabbed methods (Standard Array, Point Buy, Manual Dice Roll with max 15); Manual Dice Roll rolls 4d6 drop lowest per ability with individual reroll; Point Buy uses 27 points with standard costs; Standard Array uses fixed [15,14,13,12,10,8] with assignment dropdown; lint and typecheck pass |
