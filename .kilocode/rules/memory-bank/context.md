...
| 2026-08-24 | Added missing `choices` data to SRD class features that require player selection: Bard Magical Secrets (Lv10/14/18), Warlock Mystic Arcanum (Lv11/13/15/17), Wizard Spell Mastery (Lv18), Wizard Signature Spell (Lv20); all choices are now backed by actual interactive UI in `StepFeatureSelections`; spell choices pull from `2014_spells.json` filtered by class and level; lint and typecheck pass |
