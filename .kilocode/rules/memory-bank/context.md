...
| 2026-08-24 | Reordered creation wizard steps to: Identity → Class → Race → Abilities; updated StepIdentity to use dropdown for Background (SRD backgrounds from srd.ts) and removed Player Name; rebuilt StepAbilities with tabbed methods (Standard Array, Point Buy); removed Dice Roll tab entirely from UI; Standard Array uses fixed [15,14,13,12,10,8] with dropdown assignment and race modifier shown left of dropdown; Point Buy fully refactored with 27 points, proper +/- buttons, real-time score updates, and correct point cost calculation; lint and typecheck pass |
